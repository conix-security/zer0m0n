...
NTSTATUS sendLogs(ULONG pid, PWCHAR message, PWCHAR parameter)
{
    ...
    #ifdef DEBUG
	KdPrintEx(( DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL, "SendLogs\n"));
    #endif
	...
	KeWaitForMutexObject(&mutex, Executive, KernelMode, FALSE, NULL);
	#ifdef DEBUG
	KdPrintEx(( DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL, "\tmsg : %s\n", buf));
	#endif
    
    status = FltSendMessage(filter, &clientPort, buf, sizeBuf, NULL, 0, NULL);
	if(status == STATUS_TIMEOUT)
		KdPrintEx(( DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL, "STATUS_TIMEOUT"));
	KeReleaseMutex(&mutex, FALSE);
	ExFreePool(processName.Buffer);
	
	#ifdef DEBUG
	if(!NT_SUCCESS(status))
		KdPrintEx(( DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL, "return : 0x%08x\n", status));
	#endif DEBUG	
...
NTSTATUS ioctl_DeviceControl(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	switch(pIoStackIrp->Parameters.DeviceIoControl.IoControlCode)
	{
		...
		case IOCTL_CUCKOO_PATH:		
			...
			{
				#ifdef DEBUG
				KdPrintEx(( DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL, "IOCTL_CUCKOO_PATH : Buffer too large\n"));
				#endif DEBUG
				return STATUS_BUFFER_TOO_SMALL;
			}
				
			#ifdef DEBUG
			KdPrintEx(( DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL, "cuckooPath : %ws\n", cuckooPath));
			#endif DEBUG
